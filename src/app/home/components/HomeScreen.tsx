...
export default function HomeScreen() {
  ...
  const trendPoints = [...entries].filter((e) => e.published_at && e.views != null).sort((a, b) => new Date(a.published_at).getTime() - new Date(b.published_at).getTime());
    ...
    <div className="min-h-screen bg-creo-bg flex">
      <aside className="hidden lg:flex w-56 flex-shrink-0 flex-col border-r border-creo-border">
        ...
        <nav className="flex-1 py-3 px-2 space-y-0.5">
          {NAV.map((item) => (
            <button key={item.label} onClick={() => item.locked ? undefined : item.href && router.push(item.href)} disabled={item.locked} title={item.locked ? 'Coming soon' : undefined}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl creo-body transition-all ${item.active ? 'bg-creo-primary/12 text-creo-primary border border-creo-primary/25' : item.locked ? 'text-creo-text-muted/60 cursor-not-allowed' : 'text-creo-text-secondary hover:text-creo-text-primary hover:bg-creo-surface'}`}>
              {item.locked ? <Lock size={14} /> : <item.icon size={14} />}
              <span className="flex-1 text-left">{item.label}</span>
              {item.badge && <span className={`creo-caption px-1.5 py-0.5 rounded-full ${item.badge === 'NEW' ? 'bg-creo-success/15 text-creo-success' : 'bg-creo-warning/15 text-creo-warning'}`}>{item.badge}</span>}
            </button>
          ))}
        </nav>
      ...
      <div className="flex-1 min-w-0">
        <div className="h-14 flex items-center justify-between px-4 sm:px-6 border-b border-creo-border bg-creo-bg/95 backdrop-blur-md gap-3">
          <div className="hidden sm:flex items-center gap-2 creo-caption text-creo-text-secondary creo-surface rounded-full px-3 py-1.5">
            <span className="font-semibold text-creo-primary">Free Plan Access</span><span className="text-creo-text-muted">·</span><span>{limit === Infinity ? 'Unlimited' : limit} generations / day</span>
        ...
        <div className="p-4 sm:p-6 space-y-5 max-w-[1400px]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="creo-surface rounded-2xl p-4">
                ...
                <p className="creo-caption text-creo-text-muted">{s.label}</p><p className="text-xl font-bold text-creo-text-primary">{s.value}</p><p className="creo-caption text-creo-text-muted mt-0.5">{s.sub}</p>
              </div>
            ))}
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="creo-surface rounded-2xl p-4 lg:col-span-2">
                  ...
                  <div className="flex flex-col items-center text-center py-6 gap-2">
                    <Lock size={16} className="text-creo-text-muted" /><p className="creo-body text-creo-text-secondary">Coming soon</p>
                ...
                <div className="creo-surface rounded-2xl p-4">
                  ...
                  <div className="flex flex-col items-center text-center py-4 gap-2"><Lock size={16} className="text-creo-text-muted" /><p className="creo-caption text-creo-text-muted">Pattern insights from your content aren't available yet.</p></div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    {chats.slice(0, 3).map((c) => (
                      <button key={c.id} onClick={() => router.push('/main-app-chat-interface')} className="text-left creo-surface rounded-xl p-3 hover:border-creo-border-strong transition-all">
                        <p className="creo-body text-creo-text-secondary truncate">{c.title}</p><p className="creo-caption text-creo-text-muted mt-0.5">{c.platform} · {c.time}</p>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="creo-surface rounded-2xl p-4">
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <button key={a.label} onClick={a.action} disabled={a.locked} title={a.locked ? 'Coming soon' : undefined} className={`w-full flex items-center gap-2.5 text-left creo-surface rounded-xl p-2.5 transition-all ${a.locked ? 'opacity-50 cursor-not-allowed' : 'hover:border-creo-primary/30'}`}>
                      <a.icon size={14} className="text-creo-primary flex-shrink-0" /><div className="min-w-0"><p className="creo-body text-creo-text-secondary truncate">{a.label}</p><p className="creo-caption text-creo-text-muted truncate">{a.sub}</p></div>
                    </button>
                  ))}
                </div>
              </div>
              <div className="creo-surface rounded-2xl p-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="relative w-14 h-14 flex-shrink-0">
                    <svg viewBox="0 0 64 64" className="w-14 h-14 -rotate-90"><circle cx="32" cy="32" r="28" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="6" /><circle cx="32" cy="32" r="28" fill="none" stroke="#e17e4a" strokeWidth="6" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 28}`} strokeDashoffset={`${2 * Math.PI * 28 * (1 - genPct / 100)}`} /></svg>
          ...
          <div className="creo-surface rounded-2xl p-4">
            ...
            {!hasAnalyticsAccess ? (
              <div className="flex flex-col items-center text-center py-8 gap-2"><Crown size={18} className="text-creo-warning" /><p className="creo-body text-creo-text-secondary">Analytics is a Pro/Ultra feature.</p><button onClick={() => router.push('/upgrade')} className="creo-btn-primary mt-1 px-4 py-2 rounded-xl text-white creo-caption font-semibold">Upgrade to unlock</button></div>
            ) : loadingAnalytics ? (
              <p className="creo-body text-creo-text-muted text-center py-8">Loading...</p>
            ) : !metrics ? (
              <div className="flex flex-col items-center text-center py-8 gap-2"><TrendingUp size={18} className="text-creo-text-muted" /><p className="creo-body text-creo-text-secondary">No data yet.</p><p className="creo-caption text-creo-text-muted max-w-xs">Add your published content in the Analytics panel and real numbers will appear here.</p></div>
            ) : (
              <div className="space-y-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  ...
                  ].map((s) => (<div key={s.label} className="creo-surface rounded-xl p-3"><p className="creo-caption text-creo-text-muted">{s.label}</p><p className="text-lg font-bold text-creo-text-primary">{s.value}</p></div>))}
                </div>
                  ...
                  {trendPoints.length >= 2 ? (
                    <div className="creo-surface rounded-xl p-3">
                      ...
                      <svg viewBox="0 0 300 80" className="w-full h-20">
                        {(() => { const max = Math.max(...trendPoints.map((p) => p.views), 1); const pts = trendPoints.map((p, i) => `${(i / (trendPoints.length - 1)) * 300},${80 - (p.views / max) * 70}`).join(' '); return <polyline points={pts} fill="none" stroke="#e17e4a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />; })()}
                      </svg>
                    </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="relative w-20 h-20 flex-shrink-0">
                          <svg viewBox="0 0 64 64" className="w-20 h-20 -rotate-90">
                            {(() => { const circumference = 2 * Math.PI * 26; let offset = 0; return platformEntries.map(([platform, stats], i) => { const frac = platformTotal > 0 ? stats.totalViews / platformTotal : 0; const dash = frac * circumference; const el = (<circle key={platform} cx="32" cy="32" r="26" fill="none" stroke={DONUT_COLORS[i % DONUT_COLORS.length]} strokeWidth="8" strokeDasharray={`${dash} ${circumference - dash}`} strokeDashoffset={-offset} />); offset += dash; return el; }); })()}
                          </svg>
                          <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-xs font-bold text-creo-text-primary">{metrics.totalViews.toLocaleString()}</span><span className="creo-caption text-creo-text-muted" style={{ fontSize: 8 }}>Total Views</span></div>
                        </div>
                        <div className="space-y-1 min-w-0">
                          {platformEntries.map(([platform, stats], i) => (<div key={platform} className="flex items-center gap-1.5 creo-caption text-creo-text-secondary"><span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: DONUT_COLORS[i % DONUT_COLORS.length] }} /><span className="truncate">{platform}</span><span className="text-creo-text-muted flex-shrink-0">{platformTotal > 0 ? Math.round((stats.totalViews / platformTotal) * 100) : 0}%</span></div>))}
                        </div>
                      </div>
                    </div>
                ...
                {metrics.topByViews?.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-1.5">{metrics.topByViews.map((e: any) => (<div key={e.id} className="flex items-center justify-between creo-surface rounded-xl px-3 py-2"><span className="creo-body text-creo-text-secondary truncate">{e.title}</span><span className="creo-caption text-creo-text-muted flex-shrink-0">{(e.views || 0).toLocaleString()} views</span></div>))}</div>
                  </div>
                )}
                <div className="creo-surface rounded-xl p-4 text-center"><p className="creo-caption text-creo-text-muted">Not available for now — audience demographics need data CRÉO doesn't collect today.</p></div>
              </div>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={f.label} className="flex items-start gap-2">
                    ...
                    <div className="min-w-0"><div className="flex items-center gap-1.5"><p className="creo-caption font-semibold text-creo-text-secondary">{f.label}</p>{f.badge && <span className={`creo-caption px-1 rounded ${f.badge === 'NEW' ? 'bg-creo-success/15 text-creo-success' : 'bg-creo-warning/15 text-creo-warning'}`}>{f.badge}</span>}</div><p className="creo-caption text-creo-text-muted">{f.desc}</p></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <a href="/#why-creo" className="creo-surface rounded-2xl p-4 hover:border-creo-primary/30 transition-all block">
              <p className="creo-body font-semibold text-creo-text-primary mb-1">Why Creators Choose CRÉO</p><p className="creo-caption text-creo-text-muted mb-3">See how we compare with others.</p><span className="creo-caption text-creo-primary flex items-center gap-1">View Comparison <ChevronRight size={11} /></span>
            </a>
            <div className="creo-surface rounded-2xl p-4">
              <p className="creo-body font-semibold text-creo-text-primary mb-1">Choose Your Plan</p><p className="creo-caption text-creo-text-muted mb-3">Start free, upgrade when you're ready.</p>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={p.name} className={`creo-surface rounded-xl p-2.5 ${p.best ? 'border-creo-primary/40' : ''}`}>
                    <p className="creo-caption font-semibold text-creo-text-primary">{p.name}</p><p className="text-sm font-bold text-creo-text-primary">{p.price}</p>
            ...
            <div className="creo-surface rounded-2xl p-4">
              <p className="creo-body font-semibold text-creo-text-primary mb-1">Today's Focus</p><p className="creo-caption text-creo-text-muted mb-3">Your daily commitment to growth.</p>
              <div className="flex items-center gap-3 mb-3">
                <div className="relative w-12 h-12 flex-shrink-0">
                  <svg viewBox="0 0 48 48" className="w-12 h-12 -rotate-90"><circle cx="24" cy="24" r="20" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="4" /><circle cx="24" cy="24" r="20" fill="none" stroke="#e17e4a" strokeWidth="4" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 20}`} strokeDashoffset={`${2 * Math.PI * 20 * (1 - Math.min(1, weekDays.filter(d => d.done).length / 7))}`} /></svg>
                  <span className="absolute inset-0 flex items-center justify-center"><Flame size={13} className="text-creo-warning" /></span>
                </div>
              ...
              <div className="flex justify-between gap-1">
                {weekDays.map((d) => (
                  <div key={d.label} className="flex flex-col items-center gap-1">
                    ...
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center ${d.done ? 'bg-creo-success/20 text-creo-success' : d.isToday ? 'border border-dashed border-creo-primary/40' : 'creo-surface'}`}>{d.done && <Check size={10} />}</div>
                  </div>
                ))}
              </div>
            </div>
