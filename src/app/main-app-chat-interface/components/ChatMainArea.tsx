...
import { Menu, Sparkles, Send, ChevronDown, Download, Share2, Plus, LogOut, Crown, X } from 'lucide-react';
...
export default function ChatMainArea({ sidebarOpen, onToggleSidebar, activeChatId, onChatSaved, onNewChat }: Props) {
  ...
  const genLeft = Math.max(0, FREE_LIMIT - genCount);
    ...
    <div className="flex flex-col" style={{ height: '100dvh' }}>
      ...
      <div className="flex-shrink-0 h-14 flex items-center justify-between px-3 md:px-6 border-b border-white/5 bg-[#080812]/90 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={handleSignOut} className="w-7 h-7 rounded-lg glass border border-white/8 flex items-center justify-center text-white/40 hover:text-red-400 transition-all"><LogOut size={13} /></button>
        </div>
      </div>
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-3 md:px-6 py-4 space-y-4 overscroll-contain">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.type === 'text' && <div className={`max-w-[85%] sm:max-w-lg px-4 py-2.5 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-purple-600/80 text-white rounded-br-sm' : 'glass border border-white/8 text-white/80 rounded-bl-sm'}`}>{msg.content}</div>}
            {msg.type === 'titles' && <div className="w-full"><p className="text-white/60 text-sm mb-2">{msg.content}</p><TitleCards titles={msg.data as string[]} onSelect={handleTitleSelect} /></div>}
            {msg.type === 'hooks' && <div className="w-full"><p className="text-white/60 text-sm mb-2">{msg.content}</p><HookCards hooks={msg.data as string[]} onSelect={handleHookSelect} /></div>}
            {msg.type === 'script' && <div className="w-full"><p className="text-white/60 text-sm mb-2">{msg.content}</p><ScriptCard script={msg.data as string} /></div>}
          </div>
        ))}
        {isTyping && (
