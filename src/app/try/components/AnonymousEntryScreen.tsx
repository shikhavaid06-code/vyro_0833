...
export default function AnonymousEntryScreen() {
  ...
  return (
    <div className="min-h-screen bg-creo-bg relative overflow-hidden flex flex-col items-center justify-center px-6">
      {/* Ambient background */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-creo-primary/6 rounded-full blur-[120px]" />
      ...
      <div className={`relative z-10 w-full max-w-xl transition-all duration-500 ${stage === 'gate' || stage === 'sent' ? 'blur-sm scale-[0.98] opacity-60 pointer-events-none' : ''}`}>
        {stage === 'entry' && (
          <div className="text-center">
            <div className="inline-flex items-center gap-2 creo-surface rounded-full px-4 py-2 mb-8 border border-creo-primary/20">
              <Sparkles size={12} className="text-creo-primary" />
            ...
            <div className="creo-surface rounded-2xl border border-white/10 p-2 flex items-center gap-2 mb-3">
              <input
                ref={inputRef}
                type="text"
                value={topic}
            ...
            <button
              ...
              className="group w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full creo-btn-primary text-white font-semibold text-base hover:scale-105 active:scale-95 transition-all duration-200"
            >
        ...
        {stage === 'loading' && (
          <div className="text-center py-16 animate-fade-in relative">
            ...
            <div className="absolute left-1/2 top-16 -translate-x-1/2 w-[320px] h-[320px] bg-creo-primary/8 rounded-full blur-[90px] pointer-events-none" />

            {/* ✅ The reactor core — layered rings + orbiting sparks */}
            <div className="relative w-28 h-28 mx-auto mb-8">
              <div className="absolute inset-0 rounded-full border border-creo-primary/15" />
              ...
              <div className="absolute -inset-3 rounded-full border border-creo-primary/10 animate-ping" />
              {/* Orbiting sparks */}
              ...
              <div className="absolute inset-0 animate-spin" style={{ animationDuration: '4.5s', animationDirection: 'reverse' }}>
                <div className="absolute top-1/2 -right-1 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-creo-primary shadow-[0_0_6px_rgba(225,126,74,0.6)]" />
              </div>
              <div className="absolute inset-0 m-auto w-12 h-12 rounded-2xl bg-creo-primary/20 border border-creo-primary/30 flex items-center justify-center animate-pulse">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mt-6">
              {[{ icon: Target, label: 'Hook patterns' }, { icon: TrendingUp, label: 'Retention curves' }, { icon: Zap, label: 'Curiosity gaps' }].map(({ icon: ChipIcon, label }, i) => (
                <span key={label} className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] border transition-all duration-500 ${i <= loadingLine ? 'bg-creo-primary/15 border-creo-primary/40 text-creo-primary' : 'bg-white/[0.02] border-white/8 text-white/25'}`}>
      ...
      {(stage === 'gate' || stage === 'sent') && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 pb-4 sm:pb-0 bg-black/60 backdrop-blur-sm animate-backdrop-in">
          <div className="w-full max-w-md bg-creo-elevated border border-creo-primary/20 rounded-3xl p-7 sm:p-8 relative animate-modal-in shadow-2xl shadow-purple-500/10">
            {stage === 'gate' ? (
              <>
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-600/30 to-pink-600/30 border border-creo-primary/30 flex items-center justify-center mb-5">
                ...
                <form onSubmit={handleMagicLink} className="space-y-3">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 rounded-xl creo-surface border border-white/10 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-creo-primary/50 transition-all bg-transparent"
                    />
                  </div>
              ...
              <div className="text-center">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-600/20 to-pink-600/20 border border-creo-primary/20 flex items-center justify-center mx-auto mb-4 animate-pop-in">
                  <CheckCircle size={26} className="text-creo-primary" />
                </div>
                ...
                <p className="text-creo-primary font-semibold mb-4">{email}</p>
                ...
                <form onSubmit={handleVerifyCode} className="flex gap-2 text-left">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2.5 rounded-xl creo-surface border border-white/10 text-white text-sm tracking-[0.2em] font-mono placeholder:text-white/15 placeholder:tracking-normal focus:outline-none focus:border-creo-primary/50 transition-all bg-transparent"
                    />
                  </div>
                  <button type="submit" disabled={verifying || code.length < 6}
                    className="px-4 py-2.5 rounded-xl creo-btn-primary text-white text-xs font-semibold flex items-center gap-1.5 hover:opacity-90 active:scale-95 transition-all disabled:opacity-40">
