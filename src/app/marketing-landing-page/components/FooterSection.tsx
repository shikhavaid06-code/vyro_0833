      ...
      <div className="max-w-screen-2xl mx-auto px-6 lg:px-10">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map(({ SocialIcon, label }) => (
                <button key={label} aria-label={label} className="w-9 h-9 glass rounded-lg border border-white/8 flex items-center justify-center text-white/40 hover:text-white hover:border-white/20 transition-all duration-200"><SocialIcon /></button>
              ))}
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-white/30 mb-4">Product</p>
            <ul className="space-y-3">{['Features', 'Pricing', 'Changelog', 'Roadmap', 'API'].map((item) => (<li key={item}><a href="#" className="text-sm text-white/50 hover:text-white transition-colors duration-200">{item}</a></li>))}</ul>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-white/30 mb-4">Company</p>
            <ul className="space-y-3">{['About', 'Blog', 'Careers', 'Press Kit', 'Affiliates'].map((item) => (<li key={item}><a href="#" className="text-sm text-white/50 hover:text-white transition-colors duration-200">{item}</a></li>))}</ul>
          </div>
          <div>
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="#" className="text-xs text-white/30 hover:text-white/60 transition-colors duration-200">Cookie Policy</a>
          </div>
