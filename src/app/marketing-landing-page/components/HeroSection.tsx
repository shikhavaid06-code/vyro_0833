      ...
      <div className="relative z-10 max-w-screen-2xl mx-auto px-6 lg:px-10 text-center">
        ...
        <div className="inline-flex items-center gap-2 glass rounded-full px-4 py-2 mb-8 animate-fade-in">
          ...
          <span className="text-xs font-medium text-white/70 tracking-wide">
            {userCount?.toLocaleString()}+ creators already using VYRO
          </span>
        ...
        <p className="text-lg md:text-xl text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed font-light">
          From raw idea to scroll-stopping script in seconds. VYRO writes your titles, hooks,
          and full scripts — so you can focus on creating, not staring at a blank page.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
          <Link
            ...
            className="group flex items-center gap-2 px-8 py-4 rounded-full bg-gradient-vyro text-white font-semibold text-base glow-button hover:scale-105 active:scale-95 transition-all duration-200"
          >
            <Sparkles size={18} />
            Start Creating Free
            <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform duration-200" />
          </Link>
          <button className="group flex items-center gap-2 px-8 py-4 rounded-full glass text-white/70 hover:text-white font-medium text-base hover:bg-white/5 transition-all duration-200">
            <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-purple-500/20 transition-colors duration-200">
        ...
        <div className="max-w-3xl mx-auto glass-strong rounded-2xl p-6 text-left animate-float">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs text-white/30 font-mono">vyro.ai — content generator</span>
          </div>

          <div className="space-y-3">
            <div className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-gradient-vyro flex-shrink-0 flex items-center justify-center mt-0.5">
                <span className="text-xs font-bold text-white">Y</span>
              </div>
              <div className="glass rounded-xl rounded-tl-none px-4 py-3 text-sm text-white/70 max-w-md">
                I want to make a video about AI tools that help students study smarter
              </div>
            </div>
                ...
                <div className="glass rounded-xl rounded-tr-none px-4 py-3 text-sm text-white/80">
                  ...
                  <ul className="space-y-1.5">
                    {[
                      ...
                      <li key={`hero-title-${i}`} className="flex items-center gap-2 text-white/70">
                        <Zap size={11} className="text-pink-400 flex-shrink-0" />
                        {title}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <span className="text-xs text-white/30">VYRO is generating hooks...</span>
          </div>
        </div>
        ...
        <div className="mt-16 flex flex-col items-center gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10">
            {['YouTube', 'TikTok', 'Instagram', 'Twitch', 'Substack']?.map((platform) => (
              <span key={`platform-${platform}`} className="text-sm text-white/20 font-medium hover:text-white/40 transition-colors duration-200">
                {platform}
              </span>
            ))}
          </div>
